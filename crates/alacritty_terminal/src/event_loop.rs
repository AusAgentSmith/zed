...
pub struct EventLoop<T: tty::EventedPty, U: EventListener> {
    ...
    drain_on_exit: bool,
    ref_test: bool,
    output_tap: Option<Arc<dyn Fn(&[u8]) + Send + Sync>>,
}
    ...
    U: EventListener + Send + 'static,
{
    ...
    pub fn with_output_tap(mut self, output_tap: Arc<dyn Fn(&[u8]) + Send + Sync>) -> Self {
        ...
        X: Write,
    {
        ...
        loop {
            ...
            match self.pty.reader().read(&mut buf[unprocessed..]) {
                ...
                Ok(got) => {
                    if got > 0
                        && let Some(output_tap) = &self.output_tap
                    {
                        output_tap(&buf[unprocessed..unprocessed + got]);
                    }
                    unprocessed += got;
                },
